...
// 使用reduce方法，将上面嵌套数据扁平化；例如：prev['financial']='理财收入'
const billTypeToName = Object.keys(billListData).reduce((prev, key) => {
	billListData[key].forEach((bill) => {
		bill.list.forEach((item) => {
			// prev初始值是空对象，下列语句，经过两层循环将所有type值为键，name值作为键值，构造成对象
			prev[item.type] = item.name;
		})
	})
	return prev;
}, {});
export default billTypeToName;
